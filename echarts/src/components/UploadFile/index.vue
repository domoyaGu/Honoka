<template>
  <div style="display: flex; flex-wrap: wrap;">
    <div v-for="(item, index) in dataList" :key="item.url" class="div-img">
      <PdfPreview v-if="item.url.indexOf('.pdf') != -1" :pdfUrl="item.url"  @click="viewPdf(item.url)"  style="width: 100%; height: 100%" />
      <el-image
        v-else
  ...
  <div v-if="isViewPdf" class="pdf-div" @click="closePdf">
    <div style="width: 100%; height:20px;padding-top:60px;padding-right:60px;text-align: right">
      ...
      <div style="color:#fff;font-size:28px;width:38px;height:38px; float:right; margin-right:20px;cursor: pointer;"><svg viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg" data-v-029747aa=""><path fill="currentColor" d="m466.752 512-90.496-90.496a32 32 0 0 1 45.248-45.248L512 466.752l90.496-90.496a32 32 0 1 1 45.248 45.248L557.248 512l90.496 90.496a32 32 0 1 1-45.248 45.248L512 557.248l-90.496 90.496a32 32 0 0 1-45.248-45.248L466.752 512z"></path><path fill="currentColor" d="M512 896a384 384 0 1 0 0-768 384 384 0 0 0 0 768zm0 64a448 448 0 1 1 0-896 448 448 0 0 1 0 896z"></path></svg></div>
    </div>
    <div style="background-color: #fff;width: 50%;height: 80%" @click.stop="()=>{isViewPdf=true}">
  ...
  axios
    ...
    .then((res: any) => {
      ...
      if (res.data.code == 0) {
        ...
        pdfUrl.value = url
        if (props.typeNum) {
          imgDistinguish({ imgUrl: res.data.data, type: props.typeNum })
            .then(res2 => {
              emit('imgDataInfo', null, url);
            })
            .catch(() => {
              // emit('imgDataInfo', null, url);
            });
