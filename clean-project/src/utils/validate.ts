...
export function changePlace(place) {
  var arr
  // province省
  arr = objToArr(place[86])
  // city市 children
  for (const key in arr) {
    const code = +arr[key].value
    if (place[code]) {
      arr[key].children = objToArr(place[code])
    }
  }
  // area区 children
  for (const key in arr) {
    var city = arr[key].children
    for (const i in city) {
      const code = +city[i].value
      if (place[code]) {
        city[i].children = objToArr(place[code])
      }
    }
  }
  return arr
