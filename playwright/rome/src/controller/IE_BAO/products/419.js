      ...
      try {
        ...
        if (pageData) {
          ...
          if (employeeList.length > 3) {
            const remainderNum = employeeList.length - 3
            logger.info(remainderNum)
            for (let i = 0; i < remainderNum; i++) {
              await page.locator('.el-tab-pane').locator('.el-card__body').filter({ hasText: '保费合计' }).getByText('新增').click()
            }
          }
