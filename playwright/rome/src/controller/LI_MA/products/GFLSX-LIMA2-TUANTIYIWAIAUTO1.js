    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        if (pageData) {
          ...
          page.on('response', async (response) => {
            ...
            if (response.url()?.indexOf('/broker/organization/ocr') > -1) {
              ...
              if (industries.length === 4) {
                ...
                await page.locator('#industryCodeLevel1').selectOption(industries[1]);
              ...
              // 法人姓名
              await page.getByRole('textbox', { name: '法人姓名' }).click();
              await page.getByRole('textbox', { name: '法人姓名' }).fill(reqBody.orderGroupBean.legalPerson);
              // 纳税类型
              await page.locator('div:nth-child(5) > div:nth-child(4) > .form-control').selectOption(reqBody.orderGroupBean.organizeType);
              // 延时0.3s让页面反应，计算保费
          ...
          await page.getByPlaceholder('请输入联系人身份证号').click();
          await page.getByPlaceholder('请输入联系人身份证号').fill(reqBody.orderGroupBean.contactsIdNo || '');
          // 营业执照
          // 图片链接
  ...
  static fillProgrammeAndEmployee(page, reqBody) {
    return new Promise(async (resolve, reject) => {
      ...
      for(let i = 0; i < typeBeanList?.length; i++) {
        ...
        let name = ''
        t.typeStr = t.typeStr.replace('高空作业', '是否高空')
        switch(t.typeStr) {
          case '保险期间': 
