  ...
  static fillProgrammeAndEmployee(page, reqBody) {
    return new Promise(async (resolve, reject) => {
      ...
      for(let i = 0; i < typeBeanList?.length; i++) {
        ...
        let name = ''
        t.typeStr = t.typeStr.replace('民航客户意外', '民航客机意外').replace('住院意外津贴', '意外住院津贴')
        switch(t.typeStr) {
          case '保险期间': 
          ...
          case '伤残比例':
            // 自己系统这边命名有问题
            name = t.optionName.indexOf('行标') > -1 ? '行标10级10%起' : (t.optionName.indexOf('5%') ? '工标10级5%起' : '工标10级10%起')
            logger.info(name)
            await page.locator('.plan-item', { hasText: t.typeStr }).locator('.option', { hasText: name, exact: true }).click();
            break;
          default:
      ...
      await page.locator('.fytx_alert_btn').getByText('我已确认并了解以上内容').click();
      logger.info('——————————————————————————————基础信息——————————————————————————————————')
      // 延时0.5s让页面加载好
      await new Promise((r,rj) => setTimeout(() => {
        r(1)
