  ...
  static fillProgrammeAndEmployee(page, reqBody) {
    return new Promise(async (resolve, reject) => {
      ...
      for(let i = 0; i < typeBeanList?.length; i++) {
        ...
        let name = ''
        // t.typeStr = t.typeStr.replace('意外医疗免赔', '意外医疗免赔系数')
        switch(t.typeStr) {
          case '保险期间': 
      ...
      for (let key in occupMap) {
        ...
        await page.locator('#positionLevel1').selectOption(position[0]);
        await page.locator('#positionLevel2').selectOption(position[1]);
        await page.locator('#positionLevel3').selectOption(position[2]);
        await page.locator('#positionLevel4').selectOption(position[3]);
