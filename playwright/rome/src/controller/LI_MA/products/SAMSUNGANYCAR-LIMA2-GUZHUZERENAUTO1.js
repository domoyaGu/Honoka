  ...
  static fillProgrammeAndEmployee(page, reqBody) {
    return new Promise(async (resolve, reject) => {
      ...
      for(let i = 0; i < typeBeanList?.length; i++) {
        ...
        let name = ''
        t.typeStr = t.typeStr.replace('意外医疗免赔', '意外医疗免赔系数').replace('是否涉高','高空作业')
        switch(t.typeStr) {
          case '保险期间': 
            ...
            break;
          case '伤残比例':
            name = t.optionName.indexOf('10%') > -1 ? '10级10%起' : '10级5%起'
            logger.info(name)
            await page.locator('.plan-item', { hasText: t.typeStr }).locator('.option', { hasText: name, exact: true }).click();
            break;
          case '意外医疗免赔系数':
            name = Accident[t.optionName]
            logger.info(name)
            await page.locator('.plan-item', { hasText: t.typeStr }).locator('.option', { hasText: name, exact: true }).click();
            break;
          default:
      ...
      for (let key in occupMap) {
        ...
        for(let i = 0; i < o.employeeList.length; i++) {
          ...
          logger.info('录入')
          await page.locator('.insured_header', { hasText: `职业方案${keyIdx+1}`}).getByText('添加雇员').click();
