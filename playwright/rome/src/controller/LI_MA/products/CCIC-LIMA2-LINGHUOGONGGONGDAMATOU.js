...
const fs = require('fs');
const { Month } = require('../constant/enum.type');
      ...
      try {
        ...
        if (pageData) {
          ...
          context.route('**', async (route) => {
            if (route.request().url()?.indexOf('order_uniqid') > -1) {
              let url = route.request().url()
              logger.info(`跳转到: ${url}`)
              // 从链接里拼装返回
              let res = {
                order_uniqid: url.substring(url.indexOf('order_uniqid=')+13, url.indexOf('&sign=')),
                sign: url.substring(url.indexOf('&sign=')+6, url.length)
              }
              await page.close();
              await browser.close();
              logger.info("————————————————————————————————浏览器已关闭————————————————————————————————————")
          ...
          page.on('response', async (response) => {
            ...
            if (response.url()?.indexOf('/broker/organization/ocr') > -1) {
              ...
              logger.info('实际用工地', reqBody.orderGroupBean.workPlaceStr)
              // 实际用工地省市区
              const workPlaceArr = reqBody.orderGroupBean.workPlaceStr?.split(',') || []
              if (contactAddressArr.length === 3) {
                await page.locator('select[name="actual_operating_province"]').selectOption(workPlaceArr[0]);
                // 市辖区中文不同
                let workPlaceArr2 = ['北京市', '重庆市', '上海市'].includes(workPlaceArr[0]) ? workPlaceArr[0]+workPlaceArr[1] : workPlaceArr[1]
                await page.locator('select[name="actual_operating_city"]').selectOption(workPlaceArr2);
                await page.locator('select[name="actual_operating_district"]').selectOption(workPlaceArr[2]);
              }
              // todo:循环填写实际用工地
              // actual_operating_1_province
              // 。。。。。。
              ...
              // 企业性质
              await page.locator('div:nth-child(5) > .col-md-2 > .form-control').selectOption(reqBody.orderGroupBean.organizeType);
              // 延时0.3s让页面反应，计算保费
          ...
          if (reqBody.orderGroupBean.addressDetail) {
            await page.getByPlaceholder('请输入详细地址').nth(2).click();
            await page.getByPlaceholder('请输入详细地址').nth(2).fill(reqBody.orderGroupBean.addressDetail);
          }
          // 营业执照
          // 图片链接
      ...
      for(let i = 0; i < typeBeanList?.length; i++) {
        ...
        switch(t.typeStr) {
          case '保障期间': 
            ...
            logger.info(name)
            await page.locator('.plan-item', { hasText: t.typeStr }).getByText(name, { exact: true }).click();
            break;
          default:
            name = t.optionName
            logger.info(name)
