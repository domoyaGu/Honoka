...
class OrderController {
  ...
  static fillProgrammeAndEmployee(page, reqBody) {
    return new Promise(async (resolve, reject) => {
      ...
      for(let i = 0; i < typeBeanList?.length; i++) {
        ...
        switch(t.typeStr) {
          ...
          case '投保计划':
            name = t.optionName
            logger.info(name)
            // 根据方案A款B款取first和nth(1)
            if (reqBody.programmeDetailName.includes('A')) {
              await page.locator('.plan-item', { hasText: t.typeStr }).locator('.option', { hasText: name, exact: true }).first().click();
            } else {
              await page.locator('.plan-item', { hasText: t.typeStr }).locator('.option', { hasText: name, exact: true }).nth(1).click();
            }
            break;
          default:
            name = t.optionName
      ...
      await page.locator('.apply_btn').getByText('立即投保').click();
      logger.info('——————————————————————————————基础信息——————————————————————————————————')
      // 延时0.5s让页面加载好
      await new Promise((r,rj) => setTimeout(() => {
        r(1)
      ...
      // 暂时写死为香港
      await page.getByText(reqBody.orderGroupBean.countryStr).click();
      // 拼装职业
      let occupMap = {}
