    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        if (pageData) {
          ...
          // 开票信息
          logger.info('开票信息')
          await page.getByRole('combobox').nth(1).selectOption(reqBody.orderGroupBean.invoiceType);
    ...
    return new Promise(async (resolve, reject) => {
      ...
      for(let i = 0; i < typeBeanList?.length; i++) {
        ...
        switch(t.typeStr) {
          ...
          case '伤残比例':
            // 自己系统这边命名有问题
            name = t.optionName.indexOf('行标') > -1 ? '行标十级10%起' : (t.optionName.indexOf('5%') ? '工标10级5%起' : '工标10级10%起')
            logger.info(name)
            await page.locator('.plan-item', { hasText: t.typeStr }).locator('.option', { hasText: name, exact: true }).click();
            break;
          default:
