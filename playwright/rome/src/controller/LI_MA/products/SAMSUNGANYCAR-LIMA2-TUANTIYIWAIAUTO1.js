...
class OrderController {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        if (pageData) {
          ...
          logger.info('三星-团体意外险1-6类ABC自选套餐')
          // 认证信息
          ...
          page.on('response', async (response) => {
            ...
            if (response.url()?.indexOf('/broker/organization/ocr') > -1) {
              ...
              let addressArr = reqBody.orderGroupBean.addressStr?.split(',') || []
              if (addressArr.length === 3) {
                await page.locator('select[name="apply_province"]').selectOption(addressArr[0]);
                // 市辖区中文不同
                let address2 = ['北京市', '重庆市', '上海市'].includes(addressArr[0]) ? addressArr[0]+addressArr[1] : addressArr[1]
                await page.locator('select[name="apply_city"]').selectOption(address2);
                await page.locator('select[name="apply_district"]').selectOption(addressArr[2]);
              }
              await page.getByRole('textbox', { name: '请输入详细地址' }).first().click();
              await page.getByRole('textbox', { name: '请输入详细地址' }).first().fill(reqBody.orderGroupBean.addressDetail);
              logger.info('行业', reqBody.orderGroupBean.industry)
              const industries = reqBody.orderGroupBean.industry?.split(',') || []
              if (industries.length === 4) {
                await page.getByPlaceholder('点击选择行业类别').click();
                await page.locator('#industryCodeLevel1').selectOption(industries[0]);
                await page.locator('#industryCodeLevel2').selectOption(industries[1]);
                await page.locator('#industryCodeLevel3').selectOption(industries[2]);
                await page.locator('#industryCodeLevel4').selectOption(industries[3]);
              ...
              setTimeout(async() => {
                // 滚动到底部
                await page.evaluate(`var a=document.getElementsByClassName('fytx_alert_message')[1];a.scrollTop=a.scrollHeight;`)
                // await scrollEl.evaluate((el)=> {
                //   el.scrollTop = el.scrollHeight
                // })
                await page.locator('span', { hasText: '我已阅读并同意'}).click();
              }, 7000);
            }
          });
          // 弹窗拦截
          ...
          // 开票信息
          logger.info('开票信息')
          await page.locator('.invoice-info > div > div > .form-control').first().selectOption(reqBody.orderGroupBean.invoiceType);
          switch(reqBody.orderGroupBean.invoiceType) {
            ...
            case '2':
              ...
              logger.info(reqBody.orderGroupBean.depositBankAccount)
              ...
              // 联系人省市区、详细地址
              let contactAddressArr = reqBody.orderGroupBean.contactsAddressStr?.split(',') || []
              if (contactAddressArr.length === 3) {
                await page.locator('select[name="contact_address_province"]').selectOption(contactAddressArr[0]);
                // 市辖区中文不同
                let contactAddress2 = ['北京市', '重庆市', '上海市'].includes(contactAddressArr[0]) ? contactAddressArr[0]+contactAddressArr[1] : contactAddressArr[1]
                await page.locator('select[name="contact_address_city"]').selectOption(contactAddress2);
                await page.locator('select[name="contact_address_district"]').selectOption(contactAddressArr[2]);
              }
              if (reqBody.orderGroupBean.addressDetail) {
                await page.getByRole('textbox', { name: '请输入详细地址' }).nth(1).click();
                await page.getByRole('textbox', { name: '请输入详细地址' }).nth(1).fill(reqBody.orderGroupBean.addressDetail);
              }
              break;
            default: break;
          }
  ...
  static fillProgrammeAndEmployee(page, reqBody) {
    return new Promise(async (resolve, reject) => {
      ...
      for(let i = 0; i < typeBeanList?.length; i++) {
        ...
        switch(t.typeStr) {
          case '保险期间': 
            ...
            logger.info(name)
            await page.locator('.plan-item', { hasText: t.typeStr }).locator('.option').getByText(name, { exact: true }).click();
            break;
          default:
            name = t.optionName
            logger.info(name)
      ...
      await page.locator('#start_date').click(); // 打开聚焦
      logger.info('当前几号', sTime.substring(sTime.length - 3, sTime.length))
      await page.locator('.datepicker-days').locator('.active').click() // 点击已经选择的日期
      // 拼装职业
      let occupMap = {}
      ...
      for (let key in occupMap) {
        ...
        // （职业三个编码）
        if (keyIdx === 0) {
          await page.locator('.insured_header').getByPlaceholder('点击选择').click();
        } else {
        ...
        await page.locator('#positionLevel1').selectOption(position[2].substring(0,3));
        await page.locator('#positionLevel2').selectOption(position[2].substring(0,5));
        await page.locator('#positionLevel3').selectOption(position[2]);
        // await page.locator('#position_work_high').selectOption(o.employeeList[0].highRisk.toString()); // 是否涉高
        await page.locator('.position-btn').getByText('确定').click();
        // 职业下方的每个职业的不同保障内容
